...
function Profile() {
    ...
    async function handlePasswordChange(e){
        ...
        if(edit){
            try{
                ...
                let payload = await axios.post(`http://127.0.0.1:3001/api/users/login`, 
                {
                    email: user.email,
                    password
                })
                ...
                if(payload){
                    if(validator.isStrongPassword(newPass)){
                        if(newPass === confrimPass){
                            try{
                                const changedPass = await axios.put('http://127.0.0.1:3001/api/users/changepass',
                                {
                                    password : newPass
                                },
    ...
    async function handleDeleteUser(){
        setIsLoading(true)
        try{
            const deletedUser = await axios.delete('http://127.0.0.1:3001/api/users/delete-user',
