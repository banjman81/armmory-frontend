...
function Game() {
    ...
    async function findComments(){
        try{
            let foundComments = await axios.get(`http://127.0.0.1:3001/api/comments/find-comment/${params.id}`)
    ...
    async function handleSubmitComment(e){
        try{
            let payload = await axios.post('http://127.0.0.1:3001/api/comments/add-comment',
            {
                comment,
                id: game.id
            },
    ...
    async function handleDeleteComment(id){
        try{
            let payload = await axios.delete(`http://127.0.0.1:3001/api/comments/delete-comment/${id}`,
            {headers : {"Authorization" : `Bearer ${localStorage.getItem('loginToken')}`}})
